...
contract CoreBitcoinDualStaking is ReentrancyGuard, AccessControl, Pausable {
    ...
    function activateDualStake(
        ...
        validators[validatorId].totalCoreStaked += coreAmount;
        validators[validatorId].totalBtcStaked += btcAmount;
        ...
        epochTotalStake[currentEpoch] += (coreAmount + btcAmount);
    ...
    function unstake() external nonReentrant {
        ...
        // Calculate and harvest any pending rewards first
        (uint256 pendingCoreRewards, uint256 pendingBtcRewards) = _calculateRewards(msg.sender);
        if (pendingCoreRewards > 0 || pendingBtcRewards > 0) {
            // Update accumulated rewards
            stake.accumulatedCoreRewards += pendingCoreRewards;
            stake.accumulatedBtcRewards += pendingBtcRewards;
            stake.lastRewardClaim = block.timestamp;
            
            // Transfer rewards
            if (pendingCoreRewards > 0 && coreRewardPool >= pendingCoreRewards) {
                coreRewardPool -= pendingCoreRewards;
                coreToken.safeTransfer(msg.sender, pendingCoreRewards);
            }
            
            if (pendingBtcRewards > 0 && btcRewardPool >= pendingBtcRewards) {
                btcRewardPool -= pendingBtcRewards;
                btcToken.safeTransfer(msg.sender, pendingBtcRewards);
            }
        }
    ...
    function addRewards(
        ...
        if (coreAmount > 0) {
            coreToken.safeTransferFrom(msg.sender, address(this), coreAmount);
            coreRewardPool += coreAmount;
        }
        
        if (btcAmount > 0) {
            btcToken.safeTransferFrom(msg.sender, address(this), btcAmount);
            btcRewardPool += btcAmount;
        }
    }
